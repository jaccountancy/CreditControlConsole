    ...
    "properties": {
        ...
        "clientInformation": {
            ...
            "properties": {
                ...
                "directors": {"type": "array", "items": {"type": "string"}, "maxItems": 12},
                "personsWithSignificantControl": {"type": "array", "items": {"type": "string"}, "maxItems": 12},
                "confirmationStatementDueDate": {"type": "string"},
                "accountsFilingDueDate": {"type": "string"},
            },
        },
        "identityVerification": {
            "type": "object",
            "additionalProperties": False,
            "required": ["identityVerificationStatus", "electronicAmlStatus", "identityRisk"],
            "properties": {
                "identityVerificationStatus": {"type": "string"},
                "electronicAmlStatus": {"type": "string"},
                "identityRisk": {"type": "string"},
            },
        },
        "pepAndSanctionsScreening": {
            "type": "object",
            "additionalProperties": False,
            "required": ["pepResult", "sanctionsResult", "screeningDate", "screeningProvider", "riskLevel"],
            "properties": {
                "pepResult": {"type": "string"},
                "sanctionsResult": {"type": "string"},
        ...
        "adverseMediaScreening": {
            "type": "object",
            "additionalProperties": False,
            "required": ["adverseMediaResult", "reviewDate", "riskLevel"],
            "properties": {
                "adverseMediaResult": {"type": "string"},
        ...
        "businessRiskAssessment": {
            "type": "object",
            "additionalProperties": False,
            "required": ["industrySector", "sicCodeRisk", "cashIntensity", "regulatoryExposure", "commentary"],
            "properties": {
                "industrySector": {"type": "string"},
                "sicCodeRisk": {"type": "string"},
                "cashIntensity": {"type": "string"},
                "regulatoryExposure": {"type": "string"},
                "commentary": {"type": "string"},
            },
        },
        "geographicRisk": {
            "type": "object",
            "additionalProperties": False,
            "required": ["countryOfIncorporation", "countriesTradedWith", "fatfHighRiskExposure", "commentary", "riskLevel"],
            "properties": {
                "countryOfIncorporation": {"type": "string"},
                "countriesTradedWith": {"type": "array", "items": {"type": "string"}, "maxItems": 20},
                "fatfHighRiskExposure": {"type": "string"},
                "commentary": {"type": "string"},
                "riskLevel": {"type": "string"},
            },
        },
        "ownershipAndControl": {
            "type": "object",
            "additionalProperties": False,
            "required": ["ownershipStructure", "numberOfShareholders", "numberOfPscs", "complexity", "commentary"],
            "properties": {
                "ownershipStructure": {"type": "string"},
                "numberOfShareholders": {"type": "integer", "minimum": 0, "maximum": 2000},
                "numberOfPscs": {"type": "integer", "minimum": 0, "maximum": 2000},
        ...
        "serviceRisk": {
            "type": "object",
            "additionalProperties": False,
            "required": ["servicesProvided", "commentary", "riskLevel"],
            "properties": {
                "servicesProvided": {"type": "array", "items": {"type": "string"}, "maxItems": 20},
                "commentary": {"type": "string"},
                "riskLevel": {"type": "string"},
            },
        },
        "sourceOfFundsWealth": {
            "type": "object",
            "additionalProperties": False,
            "required": ["initialFundingSource", "directorFunding", "externalInvestment", "unusualFundingIndicators"],
            "properties": {
                "initialFundingSource": {"type": "string"},
                "directorFunding": {"type": "string"},
                "externalInvestment": {"type": "string"},
                "unusualFundingIndicators": {"type": "string"},
            },
        },
        "riskScoringMatrix": {
            "type": "object",
            ...
            "properties": {
                "identityRiskScore": {"type": "integer", "minimum": 1, "maximum": 5},
                "ownershipRiskScore": {"type": "integer", "minimum": 1, "maximum": 5},
                "geographicRiskScore": {"type": "integer", "minimum": 1, "maximum": 5},
                "serviceRiskScore": {"type": "integer", "minimum": 1, "maximum": 5},
                "industryRiskScore": {"type": "integer", "minimum": 1, "maximum": 5},
                "adverseMediaScore": {"type": "integer", "minimum": 1, "maximum": 5},
                "pepSanctionsScore": {"type": "integer", "minimum": 1, "maximum": 5},
                "overallRiskScore": {"type": "integer", "minimum": 1, "maximum": 100},
        ...
        "monitoringAndReview": {
            "type": "object",
            "additionalProperties": False,
            "required": ["initialAssessmentDate", "nextReviewDate", "triggerEvents"],
            "properties": {
...
def _database_metrics(cursor, tenant_id: str | None = None, user_id: str | None = None) -> dict:
    ...
    xero_connection = cursor.fetchone() or {}
    cursor.execute(
        """
        SELECT status, current_step, summary, error_message, customers_synced, invoices_synced,
               fetched_count, processed_count, failed_count, contacts_total, invoices_total,
    ...
    ignition_sync = cursor.fetchone() or {}
    cursor.execute(
        """
    ...
    ignition_connection = cursor.fetchone() or {}
    cursor.execute(
        """
    ...
    ignition_dataset_rows = cursor.fetchall()
    cursor.execute(
        """
        SELECT COUNT(*) FILTER (WHERE status IN ('queued', 'processing')) AS active_uploads,
    ...
    bank_upload_summary = cursor.fetchone() or {}
    cursor.execute(
        """
        SELECT COUNT(*) FILTER (WHERE status IN ('queued', 'running')) AS active_me_syncs,
               COUNT(*) FILTER (WHERE status = 'failed') AS failed_me_syncs,
    ...
    me_sync_summary = cursor.fetchone() or {}
    cursor.execute(
        """
    ...
    session_summary = cursor.fetchone() or {}
    cursor.execute(
        """
...
def _jashflow_xero_account_options(cursor, user: dict, tenant_id: str) -> list[dict]:
    cursor.execute(
        """
        SELECT DISTINCT ON (mappings.account_code)
               mappings.account_code, mappings.account_name, mappings.account_type, mappings.updated_at,
               clients.last_sync_at
        ...
        WHERE clients.user_id = %s
          AND (clients.xero_tenant_id = %s OR clients.xero_tenant_id IS NULL OR clients.xero_tenant_id = '')
          AND mappings.account_code <> ''
        ORDER BY mappings.account_code ASC, clients.last_sync_at DESC NULLS LAST, mappings.updated_at DESC
        """,
        (user["id"], tenant_id),
    )
    rows = cursor.fetchall()
    return [
        {
            ...
            "syncedAt": _iso(row.get("last_sync_at") or row.get("updated_at")) or "",
        }
        for row in rows
    ]
...
def jashflow_payload(user: dict) -> dict:
    ...
    with get_connection() as connection:
        with connection.cursor() as cursor:
            ...
            xero_accounts = _jashflow_xero_account_options(cursor, user, tenant_id)
        connection.commit()
    ...
    if bool(client.get("vat_registered_confirmed")):
        prepared = {
            **prepared,
            ...
            "vatRegisteredConfirmedAt": _iso(client.get("vat_registered_confirmed_at")) or "",
...
def _serialize_me_report_client(row: dict, mappings: list[dict], reviews: list[dict], exceptions: list[dict], reports: list[dict], submissions: list[dict]) -> dict:
    ...
    return {
        ...
        "broughtForwardTradingLossUpdatedAt": _iso(row.get("brought_forward_trading_loss_updated_at")) or "",
        "xeroContactId": row.get("xero_contact_id") or "",
        ...
        with connection.cursor() as cursor:
            cursor.execute(
                ...
                INSERT INTO me_report_clients (
                    ...
                    brought_forward_trading_loss_updated_at,
                ...
                (
                    ...
                    utcnow() if brought_forward_trading_loss else None,
...
def update_me_report_client(user: dict, client_id: str, payload: dict) -> dict:
    ...
    if "broughtForwardTradingLoss" in payload:
        brought_forward_trading_loss = _non_negative_money(payload.get("broughtForwardTradingLoss") or 0, "Brought forward trading loss")
        brought_forward_trading_loss_updated_at = now
    else:
        brought_forward_trading_loss = _money(client.get("brought_forward_trading_loss"))
        brought_forward_trading_loss_updated_at = client.get("brought_forward_trading_loss_updated_at")
    ...
    if "vatRegisteredConfirmed" in payload:
        vat_registered_confirmed_at = now if vat_registered_confirmed else None
    else:
        vat_registered_confirmed_at = client.get("vat_registered_confirmed_at")
        ...
        with connection.cursor() as cursor:
            cursor.execute(
                ...
                SET brought_forward_trading_loss = %s,
                    brought_forward_trading_loss_updated_at = %s,
    ...
    record_audit_event(
        ...
        "me_report.client_updated",
        {
            ...
            "brought_forward_trading_loss_updated_at": _iso(brought_forward_trading_loss_updated_at) or "",
    ...
    "properties": {
        ...
        "rollingVatTurnoverMonths": {
            ...
            "items": {
                "type": "object",
                "additionalProperties": False,
                "required": ["month", "turnover", "sourcePage"],
                "properties": {
                    "month": {"type": "string"},
...
async def _extract_me_report_pdf(file_bytes: bytes, filename: str, client: dict) -> dict:
    ...
    encoded = base64.b64encode(file_bytes).decode("ascii")
    prompt = (
        "Extract a month-end bookkeeping summary from this uploaded overview report PDF. "
...
ME_REPORT_S455_RATE_BEFORE_APRIL_2026 = Decimal("0.3375")
ME_REPORT_S455_RATE_FROM_APRIL_2026 = Decimal("0.3575")
ME_REPORT_PAYROLL_BASIC_PAYE_RATE = Decimal("0.20")
ME_REPORT_PAYROLL_EMPLOYEE_NI_RATE = Decimal("0.08")
ME_REPORT_PAYROLL_EMPLOYER_NI_RATE = Decimal("0.15")


def _me_report_s455_rate(period_end: date | None) -> Decimal:
    if period_end and period_end >= date(2026, 4, 6):
        return ME_REPORT_S455_RATE_FROM_APRIL_2026
    return ME_REPORT_S455_RATE_BEFORE_APRIL_2026


def _me_report_rolling_vat_months(extracted: dict, period_end: date | None, rolling_total: Decimal) -> list[dict]:
    rows = []
    for item in extracted.get("rollingVatTurnoverMonths") or []:
        if not isinstance(item, dict):
            continue
        month = str(item.get("month") or "").strip()
        amount = _money(item.get("turnover"))
        if not month and amount == 0:
            continue
        rows.append({
            "month": month or "Month",
            "turnover": float(amount),
            "source": _me_report_source_page("VAT Registration check", item.get("sourcePage")),
        })
    if rows:
        return rows[:12]
    if period_end and rolling_total:
        return [{
            "month": f"12 months to {period_end.isoformat()}",
            "turnover": float(rolling_total),
            "source": "VAT Registration check total",
        }]
    return []


def _me_report_extraction_summary(
    *,
    current_year_earnings: Decimal,
    dividend_capacity: Decimal,
    brought_forward_trading_loss: Decimal,
    trading_loss_relief_used: Decimal,
    dla_balance: Decimal,
    estimated_ct: Decimal,
    period_end: date | None,
) -> dict:
    credit_dla = max(dla_balance, Decimal("0.00"))
    overdrawn_dla = max(-dla_balance, Decimal("0.00"))
    s455_rate = _me_report_s455_rate(period_end)
    section455_tax = _money(overdrawn_dla * s455_rate)
    total_tax_exposure = _money(estimated_ct + section455_tax)
    total_potential_extraction = _money(credit_dla + max(dividend_capacity, Decimal("0.00")) + trading_loss_relief_used)
    payroll_deduction_rate = ME_REPORT_PAYROLL_BASIC_PAYE_RATE + ME_REPORT_PAYROLL_EMPLOYEE_NI_RATE
    payroll_net_rate = max(Decimal("0.01"), Decimal("1.00") - payroll_deduction_rate)
    payroll_gross_pay = _money(overdrawn_dla / payroll_net_rate) if overdrawn_dla else Decimal("0.00")
    payroll_paye_ni = _money(payroll_gross_pay - overdrawn_dla)
    payroll_employer_ni = _money(payroll_gross_pay * ME_REPORT_PAYROLL_EMPLOYER_NI_RATE)
    dividend_offset = min(overdrawn_dla, max(dividend_capacity, Decimal("0.00")))
    remaining_after_dividend = max(Decimal("0.00"), _money(overdrawn_dla - dividend_offset))
    return {
        "creditDlaAvailable": float(credit_dla),
        "overdrawnDlaExposure": float(overdrawn_dla),
        "broughtForwardTradingLossAvailable": float(brought_forward_trading_loss),
        "broughtForwardTradingLossUsed": float(trading_loss_relief_used),
        "postTaxProfits": float(current_year_earnings),
        "taxableDividendCapacity": float(max(dividend_capacity, Decimal("0.00"))),
        "totalPotentialExtraction": float(total_potential_extraction),
        "section455Rate": float(s455_rate * 100),
        "section455Tax": float(section455_tax),
        "totalTaxExposure": float(total_tax_exposure),
        "dividendOffsetAvailable": float(dividend_offset),
        "dlaAfterDividendOffset": float(remaining_after_dividend),
        "payrollMitigation": {
            "grossPayRequired": float(payroll_gross_pay),
            "employeePayeAndNi": float(payroll_paye_ni),
            "employerNi": float(payroll_employer_ni),
            "employerCost": float(_money(payroll_gross_pay + payroll_employer_ni)),
            "note": "Rough basic-rate illustration only; director annual NI, tax code, other income and payroll timing can change the result.",
        },
        "waterfall": [
            {
                "label": "Credit DLA repayment",
                "amount": float(credit_dla),
                "treatment": "Use first",
                "note": "Repay amounts owed by the company to the director before creating taxable dividends.",
            },
            {
                "label": "B/F trading loss relief used",
                "amount": float(trading_loss_relief_used),
                "treatment": "Shelters profit before dividends",
                "note": "Stored loss relief is applied to taxable profits before dividend capacity is assessed.",
            },
            {
                "label": "Further taxable dividend capacity",
                "amount": float(max(dividend_capacity, Decimal("0.00"))),
                "treatment": "Use after DLA and loss relief",
                "note": "Only use if equity and distributable reserves support it.",
            },
        ],
        "mitigationSchedule": [
            {
                "label": "Director repayment",
                "amount": float(overdrawn_dla),
                "treatment": "Cleanest fix",
                "note": "Repay the overdrawn balance into the company bank account before the s455 trigger date where possible.",
            },
            {
                "label": "Reclaim expenses or mileage",
                "amount": 0.0,
                "treatment": "Offset if evidenced",
                "note": "Post valid business expenses or mileage claims to the DLA only where receipts, mileage logs and approval exist.",
            },
            {
                "label": "Dividend offset if reserves allow",
                "amount": float(dividend_offset),
                "treatment": "Use only with sufficient reserves",
                "note": "Declare/document a dividend and credit it to the DLA only where distributable reserves and paperwork support it.",
            },
            {
                "label": "Payroll bonus illustration",
                "amount": float(payroll_gross_pay),
                "treatment": "Rough PAYE/NIC cost",
                "note": f"To net {overdrawn_dla:,.2f}, gross pay is roughly {payroll_gross_pay:,.2f}; employee PAYE/NIC about {payroll_paye_ni:,.2f} and employer NIC about {payroll_employer_ni:,.2f}.",
            },
            {
                "label": "Section 455 exposure if still overdrawn",
                "amount": float(section455_tax),
                "treatment": f"{s455_rate * 100:.2f}% cash tax exposure",
                "note": "s455 is repayable after the loan is repaid, but it is still a cash outflow to include with the CT estimate.",
            },
        ],
    }
...
def _me_report_apply_brought_forward_loss(summary: dict, client: dict) -> dict:
    ...
    result["broughtForwardTradingLossUpdatedAt"] = _iso(client.get("brought_forward_trading_loss_updated_at")) or result.get("broughtForwardTradingLossUpdatedAt") or ""
    ...
    extraction_summary = _me_report_extraction_summary(
        current_year_earnings=current_year_earnings,
        dividend_capacity=dividend_capacity,
        brought_forward_trading_loss=brought_forward_loss,
        trading_loss_relief_used=trading_loss_relief_used,
        dla_balance=dla_balance,
        estimated_ct=estimated_ct,
        period_end=period_end,
    )
    ...
    result.update({
        ...
        "section455Tax": float(section455_tax),
        "section455Rate": float(section455_rate * 100),
        "totalTaxExposure": float(total_tax_exposure),
...
def _build_me_report_pdf_summary(extracted: dict, client: dict) -> dict:
    ...
    tax_steps = [
        ...
        *([
            _me_report_step(
                "Section 455 exposure on overdrawn DLA",
                section455_tax,
                "Add to CT cash exposure",
                f"DLA overdrawn balance at {section455_rate * 100:.2f}%",
                [
                    _me_report_breakdown_item(
                        "Overdrawn director loan account",
                        max(-dla_balance, Decimal("0.00")),
                        "Balance Sheet / DLA",
                        "",
                        f"s455 at {section455_rate * 100:.2f}%",
                    )
                ],
                "Included because an overdrawn director loan account can create a repayable s455 corporation tax cash exposure.",
            ),
            _me_report_step(
                "Total CT and s455 exposure",
                total_tax_exposure,
                "Result",
                "Corporation Tax plus Section 455",
                [
                    _me_report_breakdown_item("Corporation tax estimate", estimated_ct, "CT estimate", "", "Charge"),
                    _me_report_breakdown_item("Section 455 exposure", section455_tax, "DLA balance", "", "Add"),
                ],
                "This is the total cash exposure to show the user: corporation tax plus any current s455 exposure.",
            ),
        ] if section455_tax else []),
    ]
    ...
    extraction_summary = _me_report_extraction_summary(
        current_year_earnings=current_year_earnings,
        dividend_capacity=dividend_capacity,
        brought_forward_trading_loss=brought_forward_trading_loss,
        trading_loss_relief_used=trading_loss_relief_used,
        dla_balance=dla_balance,
        estimated_ct=estimated_ct,
        period_end=period_end,
    ...
    return {
        ...
        "broughtForwardTradingLossUpdatedAt": _iso(client.get("brought_forward_trading_loss_updated_at")) or "",
        "tradingLossReliefUsed": float(trading_loss_relief_used),
        ...
        "overdrawnDlaExposure": extraction_summary["overdrawnDlaExposure"],
        "creditDlaAvailable": extraction_summary["creditDlaAvailable"],
        "dlaBalance": float(dla_balance),
        "dlaStatus": "red" if dla_balance < 0 else ("amber" if dla_balance == 0 else "green"),
        "trafficLight": traffic_light,
        "source": "uploaded_pdf",
        "forecast": forecast,
        "profitAndLossInsights": {
...
ME_REPORT_BULK_HARD_SKIP_RE = re.compile(
    r"\b(?:juk|jenius|jaccountancy|overview|report|month end|month-end|prepared by|generated by|generated on|profit and loss|balance sheet|trial balance|aged payables|aged receivables|fixed asset|depreciation|review of transactions|dashboard)\b",
    re.IGNORECASE,
)
ME_REPORT_BULK_SOFT_SKIP_RE = re.compile(
...
def _clean_me_report_bulk_client_candidate(value: str) -> str:
    candidate = re.sub(r"\s+", " ", str(value or "").replace("_", " ").replace("/", " ").replace("\\", " ")).strip()
    candidate = re.sub(r"\.pdf$", "", candidate, flags=re.IGNORECASE).strip()
    candidate = re.sub(
        r"^(?:client|client name|customer|customer name|company|company name|business|business name|prepared for|report for)\s*[:\-]\s*",
        "",
        candidate,
        flags=re.IGNORECASE,
    )
    candidate = re.sub(r"\b(?:juk|overview|report|month end|month-end|me report|pdf)\b", " ", candidate, flags=re.IGNORECASE)
    candidate = re.sub(r"^(?:for|prepared for|report for)\s+", "", candidate.strip(), flags=re.IGNORECASE)
    candidate = re.sub(r"\s+", " ", candidate).strip(" \t\r\n:-|")
    if len(candidate) < 3 or not re.search(r"[a-z]", candidate, re.IGNORECASE):
        return ""
...
def _append_me_report_bulk_candidate(candidates: list[str], seen: set[str], value: str) -> None:
    candidate = _clean_me_report_bulk_client_candidate(value)
    key = _normalise_contact_match_text(candidate)
...
def _extract_me_report_bulk_client_candidates_from_lines(lines: list[str], filename: str = "") -> list[str]:
    ...
    seen: set[str] = set()
    label_patterns = (
        re.compile(r"\b(?:client|client name|customer|customer name|company|company name|business|business name)\s*[:\-]\s*(.+)$", re.IGNORECASE),
        re.compile(r"\b(?:prepared for|report for)\s*[:\-]?\s*(.+)$", re.IGNORECASE),
    ...
    for line in lines[:30]:
        for pattern in label_patterns:
            ...
            if match:
                _append_me_report_bulk_candidate(candidates, seen, match.group(1))
    for index, line in enumerate(lines[:20]):
        if re.search(r"\b(?:overview|report|client|customer)\b", line, re.IGNORECASE):
            for next_line in lines[index + 1:index + 5]:
                _append_me_report_bulk_candidate(candidates, seen, next_line)
    for line in lines[:18]:
        _append_me_report_bulk_candidate(candidates, seen, line)
    if filename:
        filename_stem = re.sub(r"\.[^.]+$", "", filename.rsplit("/", 1)[-1])
        _append_me_report_bulk_candidate(candidates, seen, filename_stem)
    return candidates
...
def _bank_statement_quality_summary(transactions: list[dict]) -> dict:
    ordered = sorted(transactions, key=lambda row: (row.get("transaction_date") or date.min, row.get("created_at") or datetime.min.replace(tzinfo=timezone.utc)))
    previous = None
    checked_count = 0
    mismatch_count = 0
        ...
        "uploads": [
            {
                ...
                "attemptCount": int(upload.get("attempt_count") or 0),
                "sourceFileSize": int(upload.get("source_file_size") or 0),
                "canRetry": bool(upload.get("has_source_file")) and (upload.get("status") or "") != "processing",
                "lastAttemptAt": _iso(upload.get("last_attempt_at")) or "",
                "activityEvents": upload.get("activity_log") if isinstance(upload.get("activity_log"), list) else [],
        ...
        with connection.cursor() as cursor:
            ...
            if client_ids:
                ...
                if account_ids:
                    cursor.execute(
                        """
                        SELECT id, bank_account_id, filename, content_type, status, error_message,
                               source_file_size, attempt_count, last_attempt_at, activity_log,
                               statement_start_date, statement_end_date, opening_balance, closing_balance,
                               extracted_count, inserted_count, duplicate_count, created_by_user_id,
                               created_at, completed_at, source_file IS NOT NULL AS has_source_file
                        FROM bank_statement_uploads
...
def update_bank_statement_account(user: dict, bank_account_id: str, payload: dict) -> dict:
    tenant_id = _bank_statement_tenant_id(user)
    account_name = str(payload.get("accountName") or "Bank account").strip()[:160]
    nickname = str(payload.get("nickname") or "").strip()[:160]
    bank_provider = str(payload.get("bankProvider") or "").strip()[:80]
...
def _append_bank_statement_upload_activity(cursor, upload_id: str, event_type: str, message: str, detail: dict | None = None) -> None:
    cursor.execute(
        """
        UPDATE bank_statement_uploads
        SET activity_log = COALESCE(activity_log, '[]'::jsonb) || %s::jsonb
        WHERE id = %s
        """,
        (json.dumps([_bank_statement_upload_activity(event_type, message, detail)], default=_json_default), upload_id),
    )
...
def _bank_statement_pdf_chunks(file_bytes: bytes, filename: str) -> list[dict]:
    ...
    try:
        ...
        logger.warning("pypdf is unavailable; extracting bank statement as one document: %s", exc)
        return fallback
...
def _openai_incomplete_reason(payload: dict) -> str:
...
def _merge_bank_statement_extractions(extractions: list[dict]) -> dict:
    ...
    for extraction in extractions:
        ...
        chunk_label = extraction.get("label") or "statement chunk"
        chunk_index = int(extraction.get("index") or 0)
        chunk_start = _parse_optional_iso_date(extracted.get("statementStartDate"))
        chunk_end = _parse_optional_iso_date(extracted.get("statementEndDate"))
        if chunk_start and (statement_start is None or chunk_start < statement_start):
            statement_start = chunk_start
        if chunk_end and (statement_end is None or chunk_end > statement_end):
            statement_end = chunk_end
        if opening_balance is None and extracted.get("openingBalance") is not None:
            opening_balance = extracted.get("openingBalance")
        if extracted.get("closingBalance") is not None:
            closing_balance = extracted.get("closingBalance")

        for row_index, transaction in enumerate(extracted.get("transactions") or []):
            if not isinstance(transaction, dict):
                continue
            key = (
                str(transaction.get("date") or ""),
                _normalise_hash_text(transaction.get("description")),
                f"{_money(transaction.get('amount')):.2f}",
                "" if transaction.get("balance") is None else f"{_money(transaction.get('balance')):.2f}",
            )
            if key in seen_transactions:
                continue
            seen_transactions.add(key)
            enriched = dict(transaction)
            enriched["_sourceChunk"] = chunk_label
            enriched["_sourceChunkIndex"] = chunk_index
            enriched["_sourceRowIndex"] = row_index
            transactions.append(enriched)

    dated_transactions = []
    for transaction in transactions:
        parsed_date = _parse_optional_iso_date(transaction.get("date"))
        if parsed_date:
            dated_transactions.append((parsed_date, transaction))
    if dated_transactions:
        dated_transactions.sort(key=lambda item: (item[0], int(item[1].get("_sourceChunkIndex") or 0), int(item[1].get("_sourceRowIndex") or 0)))
        if statement_start is None:
            statement_start = dated_transactions[0][0]
        if statement_end is None:
            statement_end = dated_transactions[-1][0]
        transactions.sort(key=lambda row: (row.get("date") or "", int(row.get("_sourceChunkIndex") or 0), int(row.get("_sourceRowIndex") or 0)))

    if opening_balance is None:
        for transaction in transactions:
            if transaction.get("balance") is not None and transaction.get("amount") is not None:
                opening_balance = _money(transaction.get("balance")) - _money(transaction.get("amount"))
                break
    ...
    return {
        "statementStartDate": statement_start.isoformat() if statement_start else None,
        "statementEndDate": statement_end.isoformat() if statement_end else None,
        "openingBalance": float(_money(opening_balance)) if opening_balance is not None else None,
        "closingBalance": float(_money(closing_balance)) if closing_balance is not None else None,
        "accountNumber": None,
        "transactions": transactions,
        "_chunkCount": len(extractions),
    }
        ...
        with connection.cursor() as cursor:
            ...
            cursor.execute(
                ...
                INSERT INTO bank_statement_uploads (
                    bank_account_id, filename, content_type, status, source_file, source_file_size,
                    activity_log, created_by_user_id, created_at
                )
                VALUES (%s, %s, %s, 'queued', %s, %s, %s::jsonb, %s, %s)
                RETURNING id
                """,
                (bank_account_id, filename, content_type, file_bytes, len(file_bytes), json.dumps(initial_activity, default=_json_default), user["id"], utcnow()),
            )
...
def queue_bank_statement_retry(user: dict, upload_id: str) -> tuple[dict, dict, str, str, str, bytes]:
    ...
    with get_connection() as connection:
        with connection.cursor() as cursor:
            cursor.execute(
                """
                SELECT uploads.id AS upload_id, uploads.filename, uploads.content_type, uploads.status,
                       uploads.source_file, accounts.*, clients.tenant_id
                FROM bank_statement_uploads AS uploads
                JOIN bank_statement_accounts AS accounts ON accounts.id = uploads.bank_account_id
                JOIN bank_statement_clients AS clients ON clients.id = accounts.extraction_client_id
                WHERE uploads.id = %s
...
def _invoice_referenced_note_body(body: str, invoice_number: str | None) -> str:
    clean_body = str(body or "").strip()
    clean_invoice_number = str(invoice_number or "").strip()
    if not clean_body or not clean_invoice_number:
        return clean_body
    if clean_invoice_number.lower() in clean_body.lower():
        return clean_body
    return f"Invoice {clean_invoice_number}: {clean_body}"


def add_note(invoice_id: str, user: dict, body: str) -> str:
    with get_connection() as connection:
        with connection.cursor() as cursor:
            cursor.execute("SELECT invoice_number FROM invoices WHERE id = %s", (invoice_id,))
            invoice = cursor.fetchone()
            note_body = _invoice_referenced_note_body(body, invoice.get("invoice_number") if invoice else "")
            cursor.execute(
                "INSERT INTO notes (invoice_id, user_id, body) VALUES (%s, %s, %s)",
                (invoice_id, user["id"], note_body),
            )
            cursor.execute(
                "UPDATE invoices SET notes_summary = %s, updated_at = %s WHERE id = %s",
                (note_body[:200], utcnow(), invoice_id),
            )
        connection.commit()
    record_audit_event("invoice", invoice_id, "note.added", {"body": note_body}, user["id"])
    return note_body
